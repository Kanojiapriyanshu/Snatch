...
const CustomFileInput = ({ onFileChange, placeholder, iconSrc, label, fileNameKey }) => {
  const [tempFileName, setTempFileName] = useState(""); // For file name before upload
  ...
  // Only show the uploaded image and name after upload (not after file select)
  ...
  useEffect(() => {
    // Reset tempFileName and imageSrc if formData changes (e.g., after upload or refresh)
    setTempFileName("");
    setImageSrc(null);
  ...
  // Only show preview and file name if uploaded (not just selected)
  return (
    <div className="mt-4">
      <div
